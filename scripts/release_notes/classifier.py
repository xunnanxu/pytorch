...
class CommitClassifier(nn.Module):
    def __init__(self, encoder_base: torchtext.models.XLMR_BASE_ENCODER, author_map: Dict[str, int], config: CategoryConfig):
        super().__init__()
        ...
        self.dense_files = nn.Linear(config.input_dim, config.inner_dim)
    ...
    def forward(self, input_batch: CommitClassifierInputs):
        ...
        author_ids = [self.author_map.get(author, self.author_map['Unknown']) for author in authors]
...
def get_author_map(data_folder: Path, regen_data, assert_stored=False):
    if not regen_data and Path(data_folder / "author_map.pkl").exists():
        ...
        print("No author map found. Generating from scratch.")
...
def get_title_files_author_categories_zip_list(dataframe: pd.DataFrame):
    ...
    author = dataframe.author.fillna('Unknown').to_list()
...
def train(save_path: Path, data_folder: Path, regen_data: bool, resample: bool):
    ...
    commit_classifier = CommitClassifier(XLMR_BASE, author_map, classifier_config).to(device)
...
def main():
    ...
    parser.add_argument('--regen_data', action='store_true', help="Regenerate the training data, helps if labeld more examples and want to re-train.")
